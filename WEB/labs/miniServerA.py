...
class SimpleProcessor(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if self.path == '/':
            ...
            self.end_headers()
            self.wfile.write(b"<h1>Welcome to sevice1!</h1>")
            self.wfile.write(b"<button><a href='./tmp'>Show and destroy</a></button>")
        elif self.path == '/tmp':
            self.send_response(200)
            self.send_header('Content-type', 'image/jpeg')
            self.end_headers()
            with open('media/city.jpg', 'rb') as content:
                shutil.copyfileobj(content, self.wfile)
        elif self.path == "/media/city.jpg":
            self.send_response(200)
