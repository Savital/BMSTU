...
* Разработать драйвер "-" для "-". Драйвер разрабатывается для операционной системы (Windows/Linux) и должен корректно (подключаться к стеку устройств и не нарушать работу нижележащих драйверов/"-").
